...
const OWNER_ID = 'dev_2026'; // Dummy data, nantinya bisa dari Auth Store
...
const searchPlaceholder = computed(() => {
  switch (route.name) {
    ...
    case 'Profile':
      return 'Search user, games, or assets...';
    case 'Catalog Games':
    case 'Detail Games':
      return 'Search games...';
    case 'Dashboard': 
  ...
  <div class="min-h-screen bg-background text-foreground font-sans flex flex-col selection:bg-indigo-500/30">
    ...
    <header class="h-16 border-b border-border sticky top-0 bg-background backdrop-blur-md z-50 px-4 lg:px-6">
      <div class="max-w-[1400px] mx-auto w-full h-full flex items-center justify-between gap-4">
        ...
        <div 
          @click="router.push('/')" 
          class="flex items-center gap-2.5 cursor-pointer min-w-fit shrink-0 group"
        >
        ...
        <nav class="flex items-center gap-3 sm:gap-5 ml-auto">
          <router-link 
            to="/catalog" 
            class="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors hidden lg:flex"
          ...
          <BaseDropdown ref="profileDropdown" class="shrink-0 z-20">
            <template #trigger="{ isOpen }">
              ...
              >
                <User class="w-4 h-4 text-muted-foreground group-hover:text-indigo-500 transition-colors" />
              </button>
            </template>
            ...
            <template #default>
              <div class="px-3 py-2 border-b border-border mb-1 bg-muted/30">
                ...
                <p class="text-sm font-bold truncate max-w-[150px] text-foreground">{{ OWNER_ID }}</p>
              </div>
