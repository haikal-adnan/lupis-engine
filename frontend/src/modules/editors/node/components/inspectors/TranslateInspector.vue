    ...
    <div class="px-2 py-3 space-y-4">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <div class="bg-secondary/20 p-2 rounded border border-border relative overflow-hidden group">
             <div v-if="isConnected('vel_x')" class="absolute top-2 right-2 text-[9px] text-green-400 font-mono z-10 animate-pulse">
               LINKED
             </div>
             <div :class="{ 'opacity-40 pointer-events-none filter grayscale': isConnected('vel_x') }">
               ...
               <BaseNumber 
                 ...
                 class="h-8 text-xs font-mono w-full text-center bg-background/50"
               />
             </div>
          </div>

          <div class="bg-secondary/20 p-2 rounded border border-border relative overflow-hidden group">
             <div v-if="isConnected('vel_y')" class="absolute top-2 right-2 text-[9px] text-green-400 font-mono z-10 animate-pulse">
               LINKED
             </div>
      ...
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          <label class="text-xs font-medium text-muted-foreground">Use Physics</label>
          <div class="w-32">
            <BaseSelect 
              :model-value="usePhysicsValue"
              :options="booleanOptions"
              @update:model-value="(v) => updateData('use_physics', v)"
            />
          </div>
        </div>

        <div class="flex items-center justify-between">
          <label class="text-xs font-medium text-muted-foreground">Sweep (Collide)</label>
          <div class="w-32">
            <BaseSelect 
              :model-value="sweepValue"
              :options="booleanOptions"
              @update:model-value="(v) => updateData('sweep', v)"
            />
          </div>
        </div>
      </div>
...
import { Footprints, Move } from 'lucide-vue-next';
...
const usePhysicsValue = computed(() => props.node.data?.use_physics ?? false);
