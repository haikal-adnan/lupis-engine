<template>
  <div class="animate-in fade-in duration-300 space-y-10 relative">
    
    <!-- Under Development Banner Info -->
    <div class="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center gap-3">
      <div class="w-2 h-2 rounded-full bg-blue-500 animate-pulse shrink-0"></div>
      <p class="text-xs text-blue-600 dark:text-blue-400 font-medium">
        Project Settings features are currently under active development and temporarily disabled.
      </p>
    </div>

    <!-- Disabled Wrapper (Blocks interaction and reduces opacity) -->
    <div class="pointer-events-none opacity-50 select-none space-y-10 filter grayscale-[30%]">
      ...
      <section class="space-y-5">
        ...
        <div class="space-y-6">
          <div class="flex items-center gap-8 py-2">
            <!-- Added disabled prop just in case the base component supports it -->
            <BaseCheckbox v-model="autoSave" label="Enable Auto Save" box-size="w-4 h-4" icon-size="w-3 h-3" text-size="text-sm" disabled />
          </div>
            ...
            <PropertyRow label="Auto Save Interval">
              <div class="flex flex-col gap-2 w-full">
                <div class="flex items-center gap-3">
                  <BaseNumber v-model="saveInterval" prefix="Min" height="2.5rem" text-size="text-sm" label-size="text-sm" class="font-mono flex-1" :min="10" :step="5" disabled />
