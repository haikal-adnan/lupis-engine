...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 sm:p-6 select-none">
    ...
    <div class="bg-background border border-border w-full max-w-md rounded-2xl shadow-2xl flex flex-col transition-all animate-in fade-in scale-in-95 duration-200">
      ...
      <div class="p-4 border-b border-border bg-muted/30 flex justify-between items-center shrink-0 rounded-t-2xl">
        <h3 class="font-bold flex items-center gap-2 text-foreground">
          <Crop class="w-4 h-4 text-cyan-500" /> Sesuaikan Foto
        </h3>
        <button 
          @click="$emit('close')" 
          :disabled="isUploading" 
          class="text-muted-foreground hover:text-foreground hover:bg-destructive/10 hover:text-destructive p-1 rounded-lg transition-colors disabled:opacity-50"
        >
          <X class="w-5 h-5" />
        </button>
      </div>
      ...
      <div 
        ref="containerRef"
        class="relative w-full aspect-square overflow-hidden bg-checkerboard flex items-center justify-center group"
      ...
      >
        ...
        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
           <div class="w-[80%] h-[80%] rounded-full shadow-[0_0_0_9999px_rgba(0,0,0,0.5)] border-2 border-white/50 pointer-events-none relative">
      ...
      <div class="p-4 border-t border-border flex justify-between items-center bg-muted/30 rounded-b-2xl">
        ...
        <div class="flex justify-end gap-2 w-full sm:w-auto">
          <button 
            @click="$emit('close')" 
            :disabled="isUploading" 
            class="px-4 py-2 rounded-lg text-sm font-semibold hover:bg-secondary transition-colors disabled:opacity-50"
          >
            Batal
          </button>
          <button 
            @click="handleCrop" 
            :disabled="isUploading" 
            class="px-5 py-2 rounded-lg text-sm font-bold bg-cyan-500 hover:bg-cyan-600 text-white transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95 min-w-[140px]"
          >
