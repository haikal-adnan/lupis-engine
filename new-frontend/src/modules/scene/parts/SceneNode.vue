<template>
  <div class="select-none font-sans relative min-w-max">
    ...
    <div ref="dragGhostRef" class="fixed -top-[9999px] bg-foreground text-background px-2 py-1 rounded text-xs font-bold z-50 shadow-md border border-border">
      {{ node.name }}
    </div>
    ...
    <div v-if="isOpen" class="relative">
      ...
      >
        <div class="text-[10px] italic ml-2 text-muted-foreground pl-2 border-l border-dashed border-border whitespace-nowrap"
...
import { ChevronRight, ChevronDown, Layers, Folder, FolderOpen, Cuboid, Grid3X3, Type, Image, User, Ghost, Box, Lock, Eye } from 'lucide-vue-next';
...
// --- DRAG HANDLERS ---
const onDragStart = (e) => {
  if (props.node.type === 'layer') { e.preventDefault(); return; }
  e.stopPropagation();
  e.dataTransfer.effectAllowed = 'move';
  e.dataTransfer.setData('nodeId', props.node._id || props.node.id);
  if (dragGhostRef.value) e.dataTransfer.setDragImage(dragGhostRef.value, 0, 0);
};
...
const onDragOver = (e) => {
  ...
  if (percentage < 0.30) dragPosition.value = 'top';
  else if (percentage > 0.70) dragPosition.value = 'bottom';
